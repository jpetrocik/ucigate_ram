...
void TMRpcm::play(uint16_t startAddress, uint16_t length){

  if(speakerPin != lastSpeakPin){ setPin(); lastSpeakPin=speakerPin;}
  stopPlayback();

  if(!wavInfo(startAddress, length) ){ return; }//verify its a valid wav file
  sFile.open(startAddress, length);

  	playing = 1; paused = 0;
    sFile.seek(44); //skip the header info

	if(SAMPLE_RATE > 22050 ){ SAMPLE_RATE = 18000; return;}
...
boolean TMRpcm::wavInfo(uint16_t startAddress, uint16_t length){
  ...
  char wavStr[] = {'W','A','V','E'};
  for (int i =0; i<4; i++){
	  if(sFile.read() != wavStr[i]){ return 0; }
  }
    ...
    sFile.seek(34); unsigned int dVar = sFile.read();
    dVar = sFile.read() << 8 | dVar;
    if(dVar != 8){ return 0;}
